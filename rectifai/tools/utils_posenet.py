...
cache = [240] # hardcoded position of eye, More is upper
# prev_heading = ''
def get_adjacent_keypoints(keypoint_scores, keypoint_coords, min_confidence=0.1):
    results = []
    status = ["status","here"]
    for left, right in CONNECTED_PART_INDICES:
        if not (keypoint_scores[left] < min_confidence or keypoint_scores[right] < min_confidence):
            results.append(
...
def draw_skeleton_and_keypoints(
        img, instance_scores, keypoint_scores, keypoint_coords,
    ...
    for ii, score in enumerate(instance_scores):
        ...
        new_keypoints,status = get_adjacent_keypoints(
